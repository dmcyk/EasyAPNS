...
public extension String {
    func among(_ l: String, _ r: String) -> String? {
        ...
        return self[lRange.upperBound ..< rRange.lowerBound].trimmingCharacters(in: .whitespacesAndNewlines)
    }
    ...
    func flop(character: String) -> String {
        return components(separatedBy: .newlines).map {
            $0.trimmingCharacters(in: .whitespacesAndNewlines).components(separatedBy: .whitespaces)
            .filter { x in
                !x.isEmpty
            }
