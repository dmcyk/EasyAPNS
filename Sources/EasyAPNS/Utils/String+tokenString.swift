...
extension String {
  func tokenString() throws -> (privateKey: Bytes, publicKey: Bytes?) {
    ...
    guard fp != nil else { throw TokenError.errorCreatingTemporaryKeyFile }
    ...
    guard let privKeyBigNum = BN_bn2hex(bn) else { throw TokenError.dataError }
    let privateKey = "00\(String(validatingUTF8: privKeyBigNum)!)"
    var build = privateKey
    for _ in 0..<100 { build += privateKey }
